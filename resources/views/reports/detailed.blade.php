...
@section('content')
    <div class="container">
        ...
        <div class="print-header">
            ...
            <p>Del {{ \Carbon\Carbon::parse($dateFrom)->format('d/m/Y') }} al
                {{ \Carbon\Carbon::parse($dateTo)->format('d/m/Y') }}</p>
            @if($accountId)
            <p>Filtro Cuenta: {{ $accounts->find($accountId)->name }}</p> @endif
            @if($categoryId)
            <p>Filtro Categoría: {{ $categories->find($categoryId)->name }}</p> @endif
        </div>
